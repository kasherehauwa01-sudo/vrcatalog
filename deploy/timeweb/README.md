...
## Внешний nginx и экспорт Excel

Страница ошибки с подписью `nginx/... (Ubuntu)` формируется внешним nginx на VPS,
а не контейнером приложения. Поэтому одного обновления Docker-контейнеров недостаточно:
актуальный блок `location = /vr/catalog/api/export.xlsx` из `nginx.conf` должен быть
добавлен в действующий конфигурационный файл сайта на сервере. После изменения проверьте
и перечитайте конфигурацию:
...
Для маршрута экспорта настроено ожидание до 15 минут и отключена буферизация ответа.
Настройка оставлена совместимой со старыми закешированными версиями интерфейса,
которые обращаются к синхронному `/api/export.xlsx`.
