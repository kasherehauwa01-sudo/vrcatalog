...
class XMLCatalogImporter:
    ...
    def _persist_product(self, db: Session, parsed_product: Product, existing_products: dict[str, Product]) -> Product:
        ...
        if existing is None:
            # Обе даты нового товара должны соответствовать моменту первой загрузки.
            # Задаем их явно одним значением, а не полагаемся на два отдельных default.
            imported_at = datetime.utcnow()
            parsed_product.created_at = imported_at
            parsed_product.updated_at = imported_at
        ...
        if not changed:
            ...
            # поэтому фиксируем время любого фактического изменения явно. created_at
            # намеренно не меняем: это дата самой первой загрузки товара.
